	...
	$(function(){
		...
		$(window).on("resize", function(){
			...
			if(viewport.width < 925){
                if($(".main-my-schedule-tutor").hasClass('active-tab-schedule')){
                    $('.box-schedule-left').css("display","block");
                    $('#btn-open-calendar').css("display","block");
                    if(viewport.width < 650){
                        $('#tab-tutor-content .border-selectall').find('.col-md-4').css('width','52.88%');
                    }else{
                        $('#tab-tutor-content .border-selectall').find('.col-md-4').css('width','51.88%');
                    }
                }else{
                    if(viewport.width < 650){
                ...
                if($(".main-my-schedule-tutor").hasClass('active-reschedule')){
                    $(".main-my-schedule-tutor").removeClass('active-reschedule');
                }
                ...
                if($(".main-my-schedule-tutor").hasClass('active')){
                    $(".main-my-schedule-tutor").addClass('active-reschedule');
                    $('.upcoming-schedule').css("display","none");
                }
                if($('#custom-timezone').hasClass('request-page')){
...
function centerModals(){
	jQuery('.modal').each(function(i){
		...
		top = top > 0 ? top : 0;
		$clone.remove();
		if(jQuery(this).find('.modal-content').hasClass('modal-content-signup'))
			jQuery(this).find('.modal-content').css("margin-top", "-"+parseInt(top)+"px");
		else
